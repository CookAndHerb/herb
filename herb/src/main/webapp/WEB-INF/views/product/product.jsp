											...
											<div class="pi-pic">
												...
												<ul>
													...
													<li class="quick-view"><a
														href="productInfo.do?num=${list.pNum}">+ Quick View</a></li>
								...
								<ul class="pagination justify-content-center">
										...
										<c:choose>
											<c:when test="${category ne null}">
												<li class="page-item"><a class="page-link"
													href="categoryList.do?category=${category}&pageNum=${startPage-5}">◀</a>
												</li>
											</c:when>
											...
											<c:otherwise>
												<li class="page-item"><a class="page-link"
													href="product.do?pageNum=${startPage-5}">◀</a></li>
											</c:otherwise>
										</c:choose>
									</c:if>
									<!-- 페이지 숫자 -->
									<c:forEach var="i" begin="${startPage}" end="${endPage}">
										<c:choose>
											<c:when test="${category ne null}">
												<c:if test="${i==currentPage}">
													<li class="page-item active"><a class="page-link"
														href="categoryList.do?category=${category}&pageNum=${i}">${i}</a></li>
												</c:if>
												<c:if test="${i!=currentPage}">
													<li class="page-item "><a class="page-link"
														href="categoryList.do?category=${category}&pageNum=${i}">${i}</a></li>
											...
											<c:otherwise>
												<c:if test="${i==currentPage}">
													<li class="page-item active"><a class="page-link"
														href="product.do?page=${i}">${i}</a></li>
												</c:if>
												<c:if test="${i!=currentPage}">
													<li class="page-item "><a class="page-link"
														href="product.do?pageNum=${i}">${i}</a></li>
												</c:if>
											</c:otherwise>
										</c:choose>
									</c:forEach>
									...
									<c:if test="${endPage < pageCount}">
										<c:choose>
											<c:when test="${category ne null}">
												<li class="page-item"><a class="page-link"
													href="categoryList.do?category=${category}&pageNum=${startPage+5}">▶</a></li>
											</c:when>
											<c:when test="${keyword ne null}">
												<li class="page-item"><a class="page-link"
